...
namespace IFix.Editor
{
    ...
    public partial class IFixEditor
    {
        ...
        public static void RemoveInject()
        {
            var corePath = "./Assets/IFix/Plugins/IFix.Core.dll";
            AssetDatabase.ImportAsset(corePath, ImportAssetOptions.ForceUpdate);
        }
        ...
        public static void InjectAssembly(string assembly)
        {
            ...
            if (hasSomethingToDo)
            {
                ...
                var core_path = "./Assets/IFix/Plugins/IFix.Core.dll";
        ...
        public static void GenPlatformPatch(Platform platform, string patchOutputDir,
            string corePath = "./Assets/IFix/Plugins/IFix.Core.dll")
        {
            ...
            foreach (var assembly in injectAssemblys)
            {
                GenPatch(assembly, string.Format("{0}/{1}.dll", outputDir, assembly),
                    "./Assets/IFix/Plugins/IFix.Core.dll", string.Format("{0}{1}.patch.bytes", patchOutputDir, assembly));
            }
#else
        ...
        public static bool GenPatch(string assembly, string assemblyCSharpPath
            ...
            string corePath = "./Assets/IFix/Plugins/IFix.Core.dll", string patchPath = "Assembly-CSharp.patch.bytes")
        {
        ...
        public static void Patch()
        {
            ...
            try
            {
                var patch_directory = "./ExtraAssets/Patch";
                //var patch_directory = "./";
                ...
                foreach (var assembly in injectAssemblys)
                {
                    ...
                    var patch_name = string.Format("{0}.patch.bytes", assembly);
                    var patch_path = string.Format("{0}/{1}", patch_directory, patch_name);

                    if (!Directory.Exists(patch_directory))
                    {
                        Directory.CreateDirectory(patch_directory);
                    }

                    bool ret = GenPatch(assembly, assembly_path, "./Assets/IFix/Plugins/IFix.Core.dll", patch_path);
                ...
                var patchJson = JsonConvert.SerializeObject(patchFiles, Formatting.Indented);
                File.WriteAllText($"{patch_directory}/patch.json", patchJson);
            }
            catch (Exception e)
            {
                UnityEngine.Debug.LogError(e);
