...
namespace XLua
{
    ...
    public static partial class Utils
    {
        ...
        private static LuaCSFunction genFieldGetter(Type type, FieldInfo field)
        {
            if (field.IsStatic)
            {
        ...
        private static LuaCSFunction genFieldSetter(Type type, FieldInfo field)
        {
            if (field.IsStatic)
            {
                return (RealStatePtr L) =>
        ...
        private static LuaCSFunction genItemGetter(Type type, PropertyInfo[] props)
        {
        ...
        private static LuaCSFunction genItemSetter(Type type, PropertyInfo[] props)
        {
            props = props.Where(prop => !prop.GetIndexParameters()[0].ParameterType.IsAssignableFrom(typeof(string))).ToArray();
            if (props.Length == 0)
            {
        ...
        private static LuaCSFunction genEnumCastFrom(Type type)
        {
            return (RealStatePtr L) =>
            {
                try
        ...
        private static void makeReflectionWrap(RealStatePtr L, Type type, int cls_field, int cls_getter, int cls_setter,
