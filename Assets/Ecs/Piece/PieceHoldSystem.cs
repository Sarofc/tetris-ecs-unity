...
namespace Tetris
{
    sealed class PieceHoldSystem : IEcsRunSystem
    {
        readonly GameContext m_GameCtx;

        EcsFilter<PieceHoldRequest> m_Filter;
        EcsFilter<PieceMoveComponent> m_Move;
            ...
            foreach (var i in m_Filter)
            {
                ...
                foreach (var i2 in m_Move)
                {
                    ...
                    else
                    {
                        ...
                        ePiece.Get<PositionComponent>().position = new Vector3(Tetris.k_Width / 2, Tetris.k_Height);
