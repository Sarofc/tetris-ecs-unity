...
namespace Tetris
{
    sealed class PieceNextSystem : IEcsRunSystem
    {
        readonly EcsWorld _world = null;

        EcsFilter<PieceNextRequest> m_Requests;

        // TODO 改成 reactive 模式
        EcsFilter<PieceBagComponent, ComponentList<EcsEntity>> m_Bags;
        ...
        private void RequestNextBlock(ref PieceBagComponent bag, List<EcsEntity> queue)
        {
            ...
            _world.SendMessage(new PieceSpawnRequest { pieceID = cPiece.pieceID, spawnPosition = new Vector3(Tetris.k_Width / 2, Tetris.k_Height) });
        }
