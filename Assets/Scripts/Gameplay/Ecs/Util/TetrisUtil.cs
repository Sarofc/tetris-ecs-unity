...
namespace Tetris
{
    public static class TetrisUtil
    {
        ...
        public static (bool isTSpin, bool isMini) IsTSpin(EcsEntity[][] grid, in EcsEntity ePiece)
        {
            ref var cPiece = ref ePiece.Get<PieceComponent>();
            ...
            {
                // out of range check
                if (points[i].x >= TetrisDef.k_Width || points[i].x < 0 ||
                    points[i].y >= TetrisDef.k_Height + TetrisDef.k_ExtraHeight || points[i].y < 0)
                {
                    c1++;
                    c2++;
                }
