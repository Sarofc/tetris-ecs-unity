...
namespace Tetris
{
    ...
    public class Block : MonoBehaviour/*,Saro.IPoolable<Block>*/
    {
        ...
        private bool ValidChild(Vector2 move)
        {
            foreach (Transform child in transform)
            {
                var x = Mathf.RoundToInt(child.position.x + move.x);
                var y = Mathf.RoundToInt(child.position.y + move.y);
                if (!Valid(x, y)) return false;
            }
            return true;
        }
        ...
        {
            if (x >= Tetris.k_Width || x < 0 || y >= Tetris.k_Height + Tetris.k_ExtraHeight || y < 0) return false;
        ...
        private void OnDrawGizmos()
        {
            ...
            Gizmos.DrawSphere(transform.TransformPoint(rotatePoint), .1f);
        }
#endif
    }
